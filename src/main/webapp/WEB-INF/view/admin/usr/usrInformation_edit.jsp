	...
	<div class="easyui-panel" data-options="fit:true,border:false"; title="新增|修改" style="width:100%;">
	    ...
	    <form id='formEdit'>
	    	<ul class="form_layout">
	    			...
	    			<input type="hidden" name="userId" id="userId" value="${usrInformation.userId }" />
	    		
	    		<li>
	    			<span>用户编号：</span>
	    			<input type="text" name="userCode" id="userCode" value="${usrInformation.userCode }" />
	    		</li>
	    		...
	    		<li>
	    			<span>注册时间：</span>
	    			<input name="userRegtime" id="userRegtime"  class="easyui-datetimebox"
	    			data-options="required:true,showSeconds:true"
	    			value="<fmt:formatDate value='${usrInformation.userRegtime}' pattern='yyyy-MM-dd HH:mm:ss'/>"
	    			/>
	    		</li>
	    		...
	    		<li>
					...
					<select name="userIsblacklist">
						...
						<c:forEach items="${listUsrBlackView }" var="UsrBlackView">
							<option <c:if test='${UsrBlackView.userIsblacklist== usrInformation.userIsblacklist}'>selected="selected"</c:if> value="${UsrBlackView.userIsblacklist }">
