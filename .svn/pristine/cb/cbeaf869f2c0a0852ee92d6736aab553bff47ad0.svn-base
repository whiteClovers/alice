...
<script>
	...
	function editRow(id) {
		window.location.href = 'edit.do?avoidRepeatId=' + id;
	}
				...
				'Are you sure you want to delete record?', function(r) {
					if (r) {
						$.ajax({
							async : true, //是否异步
							type : "post",
							url : 'deleteAj.do?avoidRepeatId=' + id,
							success : function(data) {
...
<body>
	...
	<div id="toolbar">
		<form id="formQuery">
			...
			<div style="padding: 6px;">
				...
				<span>还款状态:</span> 
					<input name="repeatLoanStatus" id="repeatLoanStatus" style="height: 20px; border: 1px solid #ccc"> 
				<span>用户身份证:</span> 
	...
	<!-- 修改id 和 url  -->
	<table id='dataGridCustomer' class="easyui-datagrid" title="防重表管理"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'avoidRepeatId',width:180,align:'center'">防重ID</th>
				<th data-options="field:'avoidCode',width:60,align:'center'">防重编号</th>
				<th data-options="field:'repeatLoanStatus',width:60,align:'center'">还款状态</th>
				<th data-options="field:'userIdcard',width:60,align:'center'">用户身份证</th>
				<th
