...
<body>
	<div id="toolbar">
		<form id="formQuery">
			...
			<div style="padding: 6px;">
				<span>数据字典id:</span> 
					<select name="dataDictionaryId">
						<option value="">-</option>
						<c:forEach items="${listDataDictionary }" var="dataDictionary">
							<option value="${dataDictionary.dataDictionaryId }">
								${dataDictionary.dataDictionaryLabel }
							</option>
						</c:forEach>
					</select>
				<span>字典详情code:</span> 
	...
	<!-- 修改id 和 url  -->
	<table id='dataGridCustomer' class="easyui-datagrid" title="数据字典详情管理"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'dataDictionaryLabel',width:60,align:'center'">数据字典标签</th>
				<th data-options="field:'dictionaryDetailCode',width:60,align:'center'">字典详情code</th>
				<th data-options="field:'dictionaryDetailLabel',width:60,align:'center'">数据字典详情标签</th>
