...
<script>
	...
	function onclick_deleteRows() {
		var rows = $("#dataGridSysStaffRole").datagrid('getSelections');//获取选中行的所有数据
	...
	function editRow(id) {
		window.location.href = 'edit.do?staffRoleId=' + id;
	}
				...
				'Are you sure you want to delete record?', function(r) {
					if (r) {
						$.ajax({
							async : true, //是否异步
							type : "post",
							url : 'deleteAj.do?staffRoleId=' + id,
							success : function(data) {
...
<body>
	...
	<div id="toolbar">
		<form id="formQuery">
			...
			<div style="padding: 6px;">
				...
				<span>员工id:</span> 
					<input name="staffId" id="staffId"  style="border: 1px solid #ccc"> 
				<span>角色id:</span> 
					<input name="roleId" id="roleId"  style="border: 1px solid #ccc"> 
				<!-- <span>身份证:</span> 
					<input name="staffRoleIdcard" id="staffRoleIdcard"  style="border: 1px solid #ccc"> 
				<span>用户地址:</span> 
	...
	<!-- 修改id 和 url  -->
	<table id='dataGridSysStaffRole' class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'staffId',width:60,align:'center'">员工id</th>
				<th data-options="field:'roleId',width:60,align:'center'">角色id</th>
				<!-- <th data-options="field:'staffRoleIdcard',width:60,align:'center'">身份证</th>
