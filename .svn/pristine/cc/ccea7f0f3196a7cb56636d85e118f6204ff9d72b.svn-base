    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.code == 0) {
          $("#jf").html(data.credits + "分");
          $("#level").html(data.level);
          $(".container").css({"background": "url('../img/ydlbga.png') no-repeat", "background-size": "cover"});
          $("#jifenWrap").show();
          $("#jfdh").attr("href","shoppingMall.html#"+data.url);
          $("#smallPic").find("a").each(function (index, item) {
            $(this).attr("href","shoppingMall.html#"+data.url);
          })
        } /*else {
         layer.open({
         content: data.Message,
